<template>
  <div class="fundraiser">
    ...
    >
      <p slot="copytext">
        {{fundraiser.User.firstName}} will do a Contract for Good to raise money for <router-link :to='`/nonprofit/${fundraiser.Nonprofit.EIN}`'>{{ fundraiser.Nonprofit.NAME }}</router-link>.
      </p>
      <span slot="effortstext">Please sponsor {{fundraiser.User.firstName}}'s Contracts for Good</span>
    </FundraiserHero>
...
export default {
   ...
   */
  components: {
    AppHeader: () => import('Components/ContractsForGood/AppHeader.vue'),
    SharedFooter: () => import('Components/Shared/SharedFooter.vue'),
    FundraiserHeader: () => ('Components/fundraiser/FundraiserHeader.vue'),
