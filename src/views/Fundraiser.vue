<template>
  <div class="fundraiser">
    ...
    >
      <p slot="copytext">
        {{fundraiser.User.firstName}} will dip into freezing water for 3 minutes to raise money for <router-link :to='`/nonprofit/${fundraiser.Nonprofit.EIN}`'>{{ fundraiser.Nonprofit.NAME }}</router-link>.
      </p>
      <span slot="effortstext">Please sponsor {{fundraiser.User.firstName}}'s Polar Plunge for Good</span>
    </FundraiserHero>
...
export default {
   ...
   */
  components: {
    AppHeader: () => import('Components/PolarPlunge/AppHeader.vue'),
