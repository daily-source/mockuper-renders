      ...
      <div class="column is-one-third-desktop fundraiser-content__right">
        <div class="fundraiser-pledge">
          <div class="fundraiser-pledge__heading" v-if="fundraiser.participant">
            ...
            <slot name="copytext">
              <p>
                {{fundraiser.User.firstName}} will lose {{fundraiser.fundraiserDetails.hours}} pounds {{fundraiser.communityWork}} for <router-link :to="`/nonprofit/${fundraiser.Nonprofit.EIN}`">{{fundraiser.Nonprofit.NAME}}</router-link> to raise money for the same nonprofit                
              </p>
            </slot>
