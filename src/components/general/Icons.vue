<template>
  <div class="icon-wrapper">
    ...
    <svg :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 24 24" v-if="icon === 'bike'">
      <path :fill="color" d="M5,20.5A3.5,3.5 0 0,1 1.5,17A3.5,3.5 0 0,1 5,13.5A3.5,3.5 0 0,1 8.5,17A3.5,3.5 0 0,1 5,20.5M5,12A5,5 0 0,0 0,17A5,5 0 0,0 5,22A5,5 0 0,0 10,17A5,5 0 0,0 5,12M14.8,10H19V8.2H15.8L13.86,4.93C13.57,4.43 13,4.1 12.4,4.1C11.93,4.1 11.5,4.29 11.2,4.6L7.5,8.29C7.19,8.6 7,9 7,9.5C7,10.13 7.33,10.66 7.85,10.97L11.2,13V18H13V11.5L10.75,9.85L13.07,7.5M19,20.5A3.5,3.5 0 0,1 15.5,17A3.5,3.5 0 0,1 19,13.5A3.5,3.5 0 0,1 22.5,17A3.5,3.5 0 0,1 19,20.5M19,12A5,5 0 0,0 14,17A5,5 0 0,0 19,22A5,5 0 0,0 24,17A5,5 0 0,0 19,12M16,4.8C17,4.8 17.8,4 17.8,3C17.8,2 17,1.2 16,1.2C15,1.2 14.2,2 14.2,3C14.2,4 15,4.8 16,4.8Z" />
    </svg>
    <svg :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 24 24" v-if="icon === 'chart-line-variant'">
      <path :fill="color" d="M3.5,18.5L9.5,12.5L13.5,16.5L22,6.92L20.59,5.5L13.5,13.5L9.5,9.5L2,17L3.5,18.5Z" />
    </svg>
    <svg :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 24 24" v-if="icon === 'hash'">
      <path :fill="color" d="M5.41,21L6.12,17H2.12L2.47,15H6.47L7.53,9H3.53L3.88,7H7.88L8.59,3H10.59L9.88,7H15.88L16.59,3H18.59L17.88,7H21.88L21.53,9H17.53L16.47,15H20.47L20.12,17H16.12L15.41,21H13.41L14.12,17H8.12L7.41,21H5.41M9.53,9L8.47,15H14.47L15.53,9H9.53Z" />
    </svg>
    <svg :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 24 24" v-if="icon === 'heart'">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 24 24" v-if='icon === "close"'> 
      <g id="Symbol_1_1" data-name="Symbol 1 – 1" transform="translate(-768 -348)">
        <path id="Path_1" data-name="Path 1" d="M87.264,84.084l-8.159-8.159,8.159-8.159a2.249,2.249,0,0,0-3.18-3.18l-8.159,8.159-8.159-8.159a2.249,2.249,0,0,0-3.18,3.18l8.159,8.159-8.159,8.159a2.249,2.249,0,0,0,3.18,3.18l8.159-8.159,8.159,8.159a2.249,2.249,0,0,0,3.18-3.18Z" transform="translate(704.075 284.075)" :fill='color'/>
      </g>
    </svg>

    <svg xmlns="http://www.w3.org/2000/svg" :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 24.167 38.667" v-if='icon === "map-marker"'>
      <g id="iconfinder_079_Pin2_183207" transform="translate(-3)">
        <g id="Group" transform="translate(3)">
          <path id="Oval_208" data-name="Oval 208" d="M291.048,206.5c-.031-.4-.048-.8-.048-1.208C291,197.95,296.411,192,303.083,192s12.083,5.95,12.083,13.292c0,.408-.017.81-.048,1.208h.048c0,7.264-12.083,24.167-12.083,24.167S291,213.8,291,206.5Zm12.035,2.417a4.833,4.833,0,1,0-4.833-4.833A4.832,4.832,0,0,0,303.083,208.917Zm0,0" transform="translate(-291 -192)" :fill='color' fill-rule="evenodd"/>
        </g>
      </g>
    </svg>

    <svg  xmlns="http://www.w3.org/2000/svg" :style="{width: `${iconwidth}`, height: `${iconheight}`}" viewBox="0 0 64 64" v-if='icon === "profile"'>
      <g id="Group_1" data-name="Group 1">
        <circle id="Ellipse_1" data-name="Ellipse 1" cx="32" cy="32" r="32" :fill="color"/>
      </g>
      <g id="Group_3" data-name="Group 3" opacity="0.2">
        <g id="Group_2" data-name="Group 2">
          <path id="Path_1" data-name="Path 1" d="M43.9,47.543c-3.821-1.66-5.217-4.242-5.643-6.469A20.292,20.292,0,0,0,44.41,31.5a7.987,7.987,0,0,0,1.96-4.62,2.657,2.657,0,0,0-.955-2.158c-.2-8.106-5.942-14.613-13.039-14.714-.054,0-.108-.009-.163-.009-.022,0-.043,0-.065,0-7.052.039-12.783,6.41-13.125,14.409a2.676,2.676,0,0,0-1.394,2.469,8.779,8.779,0,0,0,2.663,5.448,20.261,20.261,0,0,0,5.5,8.438c-.354,2.292-1.7,5.039-5.7,6.776-2.159.938-6.1,1.781-7.808,2.649C16.649,54.962,24.911,57.962,31.876,58l.1,0A.1.1,0,0,1,32,58c7.014,0,15.325-3.01,19.713-7.808C50.01,49.324,46.063,48.481,43.9,47.543Z" fill="#231f20"/>
        </g>
      </g>
      <g id="Group_5" data-name="Group 5">
        <g id="Group_4" data-name="Group 4">
          <path id="Path_2" data-name="Path 2" d="M43.9,45.543c-3.821-1.66-5.217-4.242-5.643-6.469A20.292,20.292,0,0,0,44.41,29.5a7.987,7.987,0,0,0,1.96-4.62,2.657,2.657,0,0,0-.955-2.158c-.2-8.1-5.941-14.613-13.037-14.713-.056,0-.11-.01-.165-.01-.022,0-.043,0-.065,0-7.052.039-12.783,6.41-13.125,14.409a2.676,2.676,0,0,0-1.394,2.469,8.779,8.779,0,0,0,2.663,5.448,20.261,20.261,0,0,0,5.5,8.438c-.354,2.292-1.7,5.039-5.7,6.776-2.159.938-6.1,1.781-7.808,2.649C16.649,52.962,24.911,55.962,31.876,56l.1,0A.105.105,0,0,1,32,56c7.014,0,15.325-3.01,19.713-7.808C50.01,47.324,46.063,46.481,43.9,45.543Z" fill="#fff"/>
        </g>
      </g>
    </svg>

    <svg xmlns="http://www.w3.org/2000/svg"  :style="{width: `${iconwidth}`, height: `${iconheight}`} " viewBox="0 0 24 24" v-if='icon==="phone"'>
      <path :fill='color' d="M216,146.955a5.626,5.626,0,0,1-.17,1.2,6.773,6.773,0,0,1-.358,1.168,4.57,4.57,0,0,1-2.08,1.807,6.319,6.319,0,0,1-4.065.81,7.372,7.372,0,0,1-.98-.213q-.545-.153-.81-.247t-.946-.349q-.682-.256-.835-.307a15.437,15.437,0,0,1-2.983-1.415,28.632,28.632,0,0,1-8.182-8.182,15.438,15.438,0,0,1-1.415-2.983q-.051-.153-.307-.835t-.349-.946q-.094-.264-.247-.81a7.366,7.366,0,0,1-.213-.98,6.319,6.319,0,0,1,.81-4.065,4.57,4.57,0,0,1,1.807-2.08,6.775,6.775,0,0,1,1.168-.358,5.627,5.627,0,0,1,1.2-.17.955.955,0,0,1,.358.051q.307.1.9,1.3.188.324.511.92t.6,1.082q.273.486.528.912.051.068.3.426a4.762,4.762,0,0,1,.366.605,1.112,1.112,0,0,1,.119.486,1.415,1.415,0,0,1-.486.852,8.308,8.308,0,0,1-1.057.938,10.263,10.263,0,0,0-1.057.9,1.3,1.3,0,0,0-.486.784,1.152,1.152,0,0,0,.085.384,3.727,3.727,0,0,0,.145.349q.06.119.239.409t.2.324a16.894,16.894,0,0,0,6.972,6.972q.034.017.324.2t.409.239a3.729,3.729,0,0,0,.349.145,1.152,1.152,0,0,0,.384.085,1.3,1.3,0,0,0,.784-.486,10.261,10.261,0,0,0,.9-1.057,8.313,8.313,0,0,1,.938-1.057,1.415,1.415,0,0,1,.852-.486,1.112,1.112,0,0,1,.486.119,4.765,4.765,0,0,1,.605.366q.358.247.426.3.426.256.912.528t1.082.6q.6.324.92.511,1.193.6,1.3.9A.954.954,0,0,1,216,146.955Z" transform="translate(-192 -128)"/>
    </svg>
