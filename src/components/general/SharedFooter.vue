...
<template>
  <footer 
    id="footer"
  >
    <div class="footer__contents section">
      <div class="container">
            ...
            <template>
              <ul class="footer__navigation" v-if="footerNav.length > 0">
                <li v-for="(navLink, index) in footerNav" :key="`navLink-${index}`">
                  <a :href='navLink.href' :target='navLink.target' :class='navLink.class'>{{navLink.text}}</a>
                </li>
              </ul>
            </template>
          </div>
...
<script>
  export default {
    ...
    props: {
      imgUrl: {
        type: String,
        default: require('../../assets/img/max-good-logo-428x180.png'),
      },
      navOptions: {
        type: NavOptions,
      },
